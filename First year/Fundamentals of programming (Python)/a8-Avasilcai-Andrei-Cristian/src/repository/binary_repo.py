...
class ClientBinaryRepository:
    ...
    def _load_data(self):
        """Load clients from the binary file."""
        try:
            with open(self._file_path, "rb") as file:
                self._data = pickle.load(file)
        except FileNotFoundError:
    ...
    def _save_data(self):
        """Save the clients to the binary file."""
        try:
            with open(self._file_path, "wb") as file:
                pickle.dump(self._data, file)
        except Exception as e:
...
class RentalBinaryRepository:
    ...
    def _load_data(self):
        """Load rentals from the binary file."""
        try:
            with open(self._file_path, "rb") as file:
                self._data = pickle.load(file)
        except FileNotFoundError:
    ...
    def _save_data(self):
        """Save the rentals to the binary file."""
        try:
            with open(self._file_path, "wb") as file:
                pickle.dump(self._data, file)
        except Exception as e:
    ...
    def add(self, rental):
        ...
        if rental.book_id in self._data:
            raise DuplicateIDError(
                f"A rental with the book id {rental.book_id} already exists."
            )
